...
/**
 * 
 * A class that implements a bag of objects by using an array. The bag is
 * limited by MAX_CAPACITY.
 * 
 * @author Frank M. Carrano
 * @author Mason Walls
 * 
 * 
 *         NOTE: 0. Put your Firstname and Lastname after above empty author
 *         tag. Make sure that in both cases the first letter is uppercase and
 *         all others are lowercase. 1. You are allowed to create and use your
 *         own private helper methods. If you are introducing your own helper
 *         methods those need to be private and properly documented as per
 *         Javadoc style. 2. No additional data fields can be introduced except
 *         where it is explicitly mentioned that it is allowed. 3. No custom
 ...
 *         (Except for the methods that are provided already for you, which do
 *         not need to be implemented as part of this HW, i.e. needs to be used
 *         as it is.) 6. You are allowed to reuse any part of the provided
 *         source codes or shown under lecture notes section of Canvas, which do
 *         not violate any of above. 7. If you have any additional questions
...
public final class ResizableArrayBag<T> implements BagInterface<T>, Iterable<T> {
	...
	private class ListIteratorForResizableArrayBag implements ListIterator<T> {
		/**
		 * Data fields: You are allowed to introduce data fields, however, restrict
		 * those only to primitive types. Usage of enum is not allowed.
		 * 
		 */
		private int index;
